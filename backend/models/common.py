...
def autopad(k, p=None):  # kernel, padding
    # Pad to 'same',计算padding使得不改变图片大小
    if p is None:
...
'''
    深度分离DW卷积层：是GCONV的极端情况，分组数量等于输入通道数量，即每个通道作为一个小组分别进行卷积，结果联结作为输出，Cin = Cout = g，没有bias项
    k = 1:卷积核
    s = 1：步长stride
    gcd() 返回最大公约数
'''
...
class Bottleneck(nn.Module): # 1×1卷积层，用于降维减少计算量
    # Standard bottleneck
...
class SPP(nn.Module):
    ...
    def forward(self, x):
        x = self.cv1(x)
        # cat() 拼接list中的tensor拼接
        return self.cv2(torch.cat([x] + [m(x) for m in self.m], 1))


class Focus(nn.Module): # 将W、H信息集中到通道空间
