    ...
    while(True):
        for filename in list(core.RECEIVE_LIST.keys()):
            if(filename in core.RECEIVE_LIST.keys() and core.RECEIVE_LIST[filename]['state'] == 0):
                if ((core.RECEIVE_LIST[filename]['last_n'] + 50) < len(core.RECEIVE_LIST[filename]['blocks'])) or (core.RECEIVE_LIST[filename]['drops'] == len(core.RECEIVE_LIST[filename]['blocks'])):
