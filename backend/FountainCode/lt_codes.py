...
def getPacketSize(filesize, blocks):
    packetsize = int(filesize / blocks)
    flag = 2
    while(1):
        if flag < packetsize and flag * 2 >= packetsize:
            return flag *2
        else:
            flag = flag * 2
...
if __name__ == "__main__":
    ...
    parser.add_argument("filename", help="file path of the file to split in blocks",default='input.mp4')
    parser.add_argument("-b", "--blocks",help="block number of file",default='3',type=int)
    parser.add_argument("-r", "--redundancy", help="the wanted redundancy.", default=1.5, type=float)
    parser.add_argument("--systematic", help="ensure that the k first drops are exactaly the k first blocks (systematic LT Codes)", default=False,action="store_true")
    ...
    with open(args.filename, "rb") as file:
        ...
        file_symbols = []
        index = 0
        for curr_symbol in encode(file_blocks, drops_quantity=drops_quantity, filename=args.filename):
            # server(HOST,PORT,curr_symbol)
            if(index < 7382):
